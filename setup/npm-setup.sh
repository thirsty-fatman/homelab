...
#                         script itself (matches setup reference Phase 8).
...
#
# -----------------------------------------------------------------------------
# IMPORTANT: Back up your SSL certificate (Let's Encrypt allows only 5
# certs per exact domain set per 7 days - a reinstall wipes NPM's local
# database, but does NOT reset this rate limit. Back up after Phase 7/8
# of the setup reference, restore on the NEXT reinstall instead of
# recreating - saves your rate-limit allowance).
# -----------------------------------------------------------------------------
#
# BACKUP (run on the server, after a certificate exists in NPM):
#   mkdir -p ~/npm-backup && sudo cp -r /opt/docker/appdata/npm/data ~/npm-backup/ && sudo cp -r /opt/docker/appdata/npm/letsencrypt ~/npm-backup/ && sudo chown -R $(whoami):$(whoami) ~/npm-backup && tar -czf npm-backup.tar.gz -C ~/npm-backup . && rm -rf ~/npm-backup
#
# Then copy npm-backup.tar.gz to a synced cloud folder (PowerShell):
#   scp YOUR_USERNAME@SERVER_IP:~/npm-backup.tar.gz "YOUR_DESTINATION_PATH\"
#
